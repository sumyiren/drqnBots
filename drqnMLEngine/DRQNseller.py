...
class dqrnSeller(object):
    ...
    def restore_model(self, graph, j):
        ...
        with tf.variable_scope(self.scope):
            self.x = graph.get_tensor_by_name('SellerAgent'+str(j)+'/x:0')
            self.w_fc = self.weight_variable([lstm_size, Num_action])
            self.b_fc = self.bias_variable([Num_action])
            self.rnn_batch_size = graph.get_tensor_by_name('SellerAgent'+str(j)+'/rnn_batch_size:0')
            self.rnn_step_size  = graph.get_tensor_by_name('SellerAgent'+str(j)+'/rnn_step_size:0')
            ...
            self.output = graph.get_tensor_by_name('SellerAgent'+str(j)+'/op_to_restore:0')
